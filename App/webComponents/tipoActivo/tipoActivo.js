...
class ComponenTipoActivo extends HTMLElement {
  constructor() {
    super();
    this.render();
    this.detectarClick();
  ...
  datosFormulario() {
    ...
    btnGuardar.addEventListener("click", async (e) => {
      ...
      if(btnGuardar.value === "Registrar tarea"){
        guardarDatos(formulario, "tipyAssets");
        console.log("New")
      }else if (btnGuardar.value === "Actualizar"){
        const id = btnGuardar.id
        actualizarDatos(formulario, "tipyAssets", id)
  ...
  async renderUpdatedData() {
    ...
    datosDiv.innerHTML = "";
    const datos = await getData("tipyAssets");
    datos.forEach((dato) => {
      datosDiv.innerHTML += `
      <div class="card-estado">
  ...
  actualizarEstado() {
    ...
    areaPrincipal.addEventListener("click", async (e) => {
      if (e.target.classList.contains("editar")) {
        const id = e.target.id;
        const estado = await getOneData(id, "tipyAssets");
        const dialog = document.querySelector("dialog");
        const btnGuardar = document.querySelector(".guardar");
...
customElements.define('tipo-activo', ComponenTipoActivo)